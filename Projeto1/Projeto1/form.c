...
Form newSquare(float x, float y, float side) {
	...
	//return newForm(SQUARE, x - (side / 2), y - (side / 2), x + (side / 2), y + (side / 2));
}
...
Form newLineC(float x, float y, float xSide, float ySize) {
	return newFormC(LINE, x, y, xSide, ySize);
}
...
Form newTriangleEquiC(float x, float y, float xSize, float ySize) {
	return newFormC(TRIANGLE_EQUI, x, y, xSize, sqrt(3) / 2 * xSize);
}
...
Form createForm(int x, int y, int maxSize) {
	...
	case 2:
		...
		break;
	case 3:
		f = newLine(x, y, rand() % (maxSize / 4), rand() % (maxSize / 4));
		break;
	case 4:
		f = newTriangleEqui(x, y, rand() % (maxSize / 4), rand() % (maxSize / 4));
		break;
	case 5:
		f = newHexagon(x, y, rand() % (maxSize / 4), rand() % (maxSize / 4));
		break;
	case 6:
		f = newCircumference(x, y, rand() % (maxSize / 4), rand() % (maxSize / 4));
		break;
...
Form createFormC(int type, int x, int y, int maxSize) {
	...
	case TRIANGLE_ISO:
		...
		break;
	case LINE:
		f = newLineC(x, y, rand() % (maxSize / 4), rand() % (maxSize / 4));
		break;
	case TRIANGLE_EQUI:
		f = newTriangleEquiC(x, y, rand() % (maxSize / 4), rand() % (maxSize / 4));
		break;
	case HEXAGON:
		f = newHexagon(x, y, rand() % (maxSize / 4), rand() % (maxSize / 4));
		break;
